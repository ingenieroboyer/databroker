...
## Añadimos el usuario del nt

acdir=pd.read_csv("/home/tecnoboy/CRECER/trato_data/data/trabajadadores/ModeloDatos/active_directory.csv")
acdir['email']=acdir['Mail'].str.lower()
acdir['ntuser2']=acdir['Match Ntid'].str.lower()



empleados2= pd.merge(empleados, acdir, on=['email'], how='left')
# empleados2=empleados2.lab_union.fillna('0')
empleados2=empleados2.drop([ 'Apellido', 'Nombre', 'Ntid', 'Mail', 'Match Ntid', 'Match Mail',
       'Accion', 'Nota'], axis=1)

## Se pasa a mongo 
collection = db.workers5
collection.insert_many(empleados2.to_dict('records'))
